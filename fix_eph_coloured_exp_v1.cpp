...
// internal headers
#include "fix_eph_coloured_exp_v1.h"
#include "eph_beta.h"
#include "eph_fdm.h"
...
// constructor
FixEPHColouredExpV1::FixEPHColouredExpV1(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {
...
// destructor
FixEPHColouredExpV1::~FixEPHColouredExpV1() {
  delete random;
  delete[] type_map;
...
void FixEPHColouredExpV1::init_list(int id, NeighList *ptr) {
  this->list = ptr;
}

int FixEPHColouredExpV1::setmask() {
  int mask = 0;
  mask |= POST_FORCE;
  mask |= END_OF_STEP;
  /* integrator functionality */
...
/* integrator functionality */
void FixEPHColouredExpV1::initial_integrate(int) {
...
void FixEPHColouredExpV1::final_integrate() {
  if(eph_flag & Flag::NOINT) return;
...
void FixEPHColouredExpV1::end_of_step() {
  double **x = atom->x;
  double **v = atom->v;
  int *type = atom->type;
  int *mask = atom->mask;
...
void FixEPHColouredExpV1::calculate_environment() {
  double **x = atom->x;
  int *type = atom->type;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixEPHColouredExpV1::force_prl() {
  double **x = atom->x;
  double **v = atom->v;
  int *type = atom->type;
  int *mask = atom->mask;
...
void FixEPHColouredExpV1::post_force(int vflag) {
  double **f = atom->f;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  int *numneigh = list->numneigh;
...
void FixEPHColouredExpV1::reset_dt() {
  eta_factor = sqrt(2.0 * force->boltz / update->dt);
  zeta_factor = 1.0 - exp(- update->dt / tau0);
...
void FixEPHColouredExpV1::grow_arrays(int ngrow) {
  n = ngrow;
...
double FixEPHColouredExpV1::compute_vector(int i) {
  if(i == 0)
    return Ee;
  else if(i == 1) {
...
int FixEPHColouredExpV1::pack_forward_comm(int n, int *list, double *data, int pbc_flag, int *pbc) {
  int m;
  m = 0;
  switch(state) {
    case FixState::RHO:
...
void FixEPHColouredExpV1::unpack_forward_comm(int n, int first, double *data) {
  int m, last;
  m = 0;
  last = first + n;
...
/** TODO **/
double FixEPHColouredExpV1::memory_usage() { return 0; }

/* save temperature state after run */
void FixEPHColouredExpV1::post_run() {
  if(myID == 0) fdm.save_state(T_state);
}

int FixEPHColouredExpV1::pack_exchange(int i, double *buf) {
  size_t m = 0;
  buf[m++] = zi_i[i][0];
  buf[m++] = zi_i[i][1];
  buf[m++] = zi_i[i][2];
...
int FixEPHColouredExpV1::unpack_exchange(int nlocal, double *buf) {
  size_t m = 0;
  zi_i[nlocal][0] = buf[m++];
  zi_i[nlocal][1] = buf[m++];
  zi_i[nlocal][2] = buf[m++];
...
void FixEPHColouredExpV1::copy_arrays(int i, int j, int) {
  zi_i[j][0] = zi_i[i][0];
  zi_i[j][1] = zi_i[i][1];
  zi_i[j][2] = zi_i[i][2];
